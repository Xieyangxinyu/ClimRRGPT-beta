    ...
    if "literature_review_summary" in st.session_state:
        ...
        for i in range(2):
            ...
            with st.expander("Show Literature Search Results"):
                st.write(st.session_state.retrieved_literature[i])
            messages = [
                {"role": "system", "content": st.session_state.config["literature_review_instructions"][0]['content'].format(retrieve_literature = literature_search(st.session_state.questions[i]))}, 
